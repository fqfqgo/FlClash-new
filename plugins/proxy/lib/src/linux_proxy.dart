...
class LinuxProxy {
  ...
  Future<bool> start(
    ...
    return _verify(port, homeDir, selection);
  }
  ...
  Future<bool> _verify(
    int port,
    String homeDir,
    _LinuxBackendSelection selection,
  ) async {
    if (selection.backend == LinuxProxyBackend.kde) {
      final reader = selection.executable.replaceFirst(
        'kwriteconfig',
        'kreadconfig',
      );
  ...
  Future<bool> _outputEquals(
    String executable,
    List<String> arguments,
    String expected,
  ) async {
    try {
      final result = await _commandRunner.process(executable, arguments);
      if (result.exitCode != 0) return false;
      var value = result.stdout.toString().trim();
      if (value.length >= 2 &&
